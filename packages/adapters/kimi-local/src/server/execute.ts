...
async function stageKimiProjectAssets(input: {
  ...
  if (input.instructionsFilePath) {
    if (!await pathExists(instructionsTarget)) {
      ...
      await input.onLog(
        "stdout",
        `[paperclip] Kimi workspace already contains ${instructionsTarget}; leaving it unchanged.\n`,
      );
    }
  } else {
  ...
  if (selectedSkills.length > 0) {
    const kimiDir = path.join(input.cwd, ".kimi");
    const skillsRoot = path.join(kimiDir, "skills");
    if (!await pathExists(kimiDir)) {
      await fs.mkdir(kimiDir, { recursive: true });
      ensureCleanupDir(kimiDir);
    }
    ...
    for (const skill of selectedSkills) {
      ...
      if (await pathExists(target)) {
        await input.onLog(
          "stdout",
          `[paperclip] Kimi skill target already exists at ${target}; leaving it unchanged.\n`,
        );
        continue;
      }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const kimiSkillEntries = await readPaperclipRuntimeSkillEntries(config, import.meta.url);
  const desiredKimiSkillNames = resolvePaperclipDesiredSkillNames(config, kimiSkillEntries);
  const instructionsFilePath = asString(config.instructionsFilePath, "").trim();
  const stagedAssets = await stageKimiProjectAssets({
    cwd,
    instructionsFilePath,
    skillEntries: kimiSkillEntries,
    desiredSkillNames: desiredKimiSkillNames,
    onLog,
  });
  ...
  try {
    ...
    if (executionTargetIsRemote) {
      await onLog(
        "stdout",
        `[paperclip] Syncing Kimi workspace to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
    ...
    const commandNotes = (() => {
      const notes: string[] = ["Prompt is passed to Kimi via --print in headless mode."];
      if (stagedAssets.stagedInstructionsPath) {
        notes.push(`Staged project instructions at ${stagedAssets.stagedInstructionsPath} for native Kimi discovery.`);
      }
      if (stagedAssets.stagedSkillsCount > 0) {
        notes.push(`Staged ${stagedAssets.stagedSkillsCount} Paperclip skill(s) into .kimi/skills for native Kimi discovery.`);
      }
      return notes;
    })();
